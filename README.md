...
```powershell
python .\Query.py -c WebBrowser -c Development --out refs
```
...
```powershell
python .\Query.py -c WebBrowser -c Development --merge-to Browsers+Dev.refs --out refs
```
...
```powershell
python .\Download.py --refs-file "D:\refs\WebBrowser.refs" --out "D:\flatpakrefs"
```

From all .refs files in a directory:
```powershell
python .\Download.py --refs-dir "D:\refs" --out "D:\flatpakrefs"
```

Combine multiple refs files explicitly:
```powershell
python .\Download.py -f refs\WebBrowser.refs -f refs\Development.refs --out out
```
